...
void registrarSocio(string ci, string nombre, DtFecha *fechaIngreso,
                    DtMascota *dtMascota);
...
int main() {
  ...
  int edadMascota, generoMascota, tipoMascota, tipoPelo, vacunas, raza, dia,
      mes, anio, cantConsultas, cantMascotas;
  float pesoMascota;
  DtFecha *dtF;
  ...
  while (opcion != 0) {
    ...
    switch (opcion) {
    case 1:
      cout << "\nPara comenzar primero indique la fecha del día de hoy, "
              "comenzando por el día: (--/xx/xxxx) ";
      cin >> dia;
      ...
      if (tipoMascota) {
        cout << "\n-Ingrese el tipo de pelo:\n  -1 Corto\n  -2 Mediano\n  -3 "
                "Largo\n";
        cin >> tipoPelo;
        DtGato *dtG = new DtGato(nombreMascota, generoMascota, pesoMascota,
                                 tipoMascota, tipoPelo);
        ...
        cout << "\nSeleccione la raza:\n  1- Labrador\n  2- Ovejero\n  3- "
                "Bulldog\n  4- Pitbull\n  5- Collie\n  6- Pekines\n  7- Otro\n";
        cin >> raza;
        DtPerro *dtP = new DtPerro(nombreMascota, generoMascota, pesoMascota,
                                   tipoMascota, raza, auxV);
...
void registrarSocio(string ci, string nombre, DtFecha *fechaIngreso,
  ...
  if (s.tope == MAX_SOCIOS)
    ...
    int posSocio = s.tope;
    Socio *S = new Socio(nombre, ci, fechaIngreso->getDia(),
                         fechaIngreso->getMes(), fechaIngreso->getAnio());
    s.socios[posSocio] = S;
    s.tope++;
    if (dtMascota->getPerroGato()) {
      DtGato *dtgato = dynamic_cast<DtGato *>(dtMascota);
      Gato *catA = new Gato(dtgato->getNombreDtMsc(), dtgato->getDtPeso(),
                            dtgato->getDtGenero(), dtgato->getPerroGato(),
                            dtgato->getTipoPelo());
      s.socios[posSocio]->setArrM(s.socios, posSocio,
                                  catA); // agrega el aux en la posicion topeM
                                         // del socio i y suma 1 al topeM
    } else {
      DtPerro *dtperro = dynamic_cast<DtPerro *>(dtMascota);
      Perro *dogA = new Perro(dtperro->getNombreDtMsc(), dtperro->getDtPeso(),
                              dtperro->getDtGenero(), dtperro->getPerroGato(),
                              dtperro->getRaza(), dtperro->getVC());
      s.socios[posSocio]->setArrM(s.socios, posSocio,
                                  dogA); // agrega el aux en la posicion topeM
                                         // del socio i y suma 1 al topeM
    }
  }
}
  ...
  if (posSocio = s.tope)
    ...
    if (j <= 9) {
      if (dtMascota->getPerroGato()) {
        ...
        Gato *catB = new Gato(gato->getNombreDtMsc(), gato->getDtPeso(),
                              gato->getDtGenero(), gato->getPerroGato(),
                              gato->getTipoPelo());
        s.socios[posSocio]->setArrM(s.socios, posSocio,
                                    catB); // agrega el aux en la posicion topeM
                                           // del socio i y suma 1 al topeM
      } else {
        DtPerro *perro = dynamic_cast<DtPerro *>(dtMascota);
        Perro *dogB = new Perro(perro->getNombreDtMsc(), perro->getDtPeso(),
                                perro->getDtGenero(), perro->getPerroGato(),
                                perro->getRaza(), perro->getVC());
        s.socios[posSocio]->setArrM(s.socios, posSocio,
                                    dogB); // agrega el aux en la posicion topeM
                                           // del socio i y suma 1 al topeM
      }
    } else
...
void ingresarConsulta(string motivo, string ci, DtFecha *fechaConsulta) {
  ...
  if (j <= 20) {
    Consulta *C =
        new Consulta(motivo, fechaConsulta->getDia(), fechaConsulta->getMes(),
                     fechaConsulta->getAnio());
...
DtConsulta **verConsultasAntesDeFecha(DtFecha *fecha, string ciSocio,
  ...
  while (posSocio < s.tope && s.socios[posSocio]->getCI() != ciSocio)
    posSocio++;
  DtConsulta **result =
      s.socios[posSocio]->obtenerArrC(s.socios, posSocio, fecha, cantConsultas);
  return result;
}
...
DtMascota **obtenerMascotas(string ci, int &cantMascotas) {
  ...
  while (posSocio < s.tope && s.socios[posSocio]->getCI() != ci)
    posSocio++;
  DtMascota **result =
      s.socios[posSocio]->obtenerArrM(s.socios, posSocio, cantMascotas);
  return result;
}
