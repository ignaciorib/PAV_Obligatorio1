...
int Socio::getTopeM() { return topeM; }

int Socio::getTopeC() { return topeC; }
...
void Socio::setArrM(Socio *s[], int i, Mascota *auxM) {
  ...
  for (int g = 0; g < topeM; g++) {
    cout << "\n\n"
         << s[i]->m[g]->getGenero() << "  " << s[i]->m[g]->getPeso() << "  "
         << s[i]->m[g]->obtenerRD() << "\n";
  }
}
...
DtConsulta **Socio::obtenerArrC(Socio *s[], int posSocio, DtFecha *f,
                                int &cont) {
  ...
  if (cont > 0) {
    ...
    for (int iter = 0; iter < s[posSocio]->topeC; iter++) {
      if (comp(s[posSocio]->c[iter]->getFechaConsulta(), f)) {
        result[iter] = new DtConsulta(s[posSocio]->c[iter]->getFechaConsulta(),
                                      s[posSocio]->c[iter]->getMotivo());
      }
    }
    return result;
  } else
  ...
  if (cont > 0) {
    ...
    for (int iter = 0; iter < cont; iter++) {
      if (s[posSocio]->m[iter]->getPerroGato()) {
        ...
        DtGato *auxGato =
            new DtGato(catC->getNombreMsc(), catC->getGenero(), catC->getPeso(),
                       catC->getPerroGato(), catC->getTipoPelo());
        result[iter] = auxGato;
        auxGato->mostrarInfDtGato();
      } else {
        Perro *dogC = dynamic_cast<Perro *>(s[posSocio]->m[iter]);
        DtPerro *auxPerro = new DtPerro(dogC->getNombreMsc(), dogC->getGenero(),
                                        dogC->getPeso(), dogC->getPerroGato(),
                                        dogC->getRaza(), dogC->getVacu());
